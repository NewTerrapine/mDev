...
func main() {
	...
	if err := e.Run(&games.ExampleGame{}); err != nil {
		if err.Error() == "quit" { os.Exit(0) }
		slog.Error("game error", "err", err)
		os.Exit(1)
	}
}
EOF
...
func (e *Engine) Run(g Game) error {
	e.Game = g
	if err := g.Load(e); err != nil { return err }
	const targetFPS = 60
...
func (c *Console) Set(x, y int, r rune) { if x >= 0 && x < c.width && y >= 0 && y < c.height { c.buffer[y*c.width+x] = r } }
func (c *Console) Draw(ents []ecs.Entity) {
	for _, e := range ents {
		if p, ok := e.Components["position"].(*ecs.Position); ok {
			if v, ok := e.Components["visual"].(*ecs.Visual); ok {
				c.Set(p.X, p.Y, v.Char)
			}
		}
	}
}
func (c *Console) Present() {
	...
	for y := 0; y < c.height; y++ {
		for x := 0; x < c.width; x++ { s.WriteRune(c.buffer[y*c.width+x]) }
		s.WriteByte('\n')
	}
	fmt.Print("\033[H")
	fmt.Print(s.String())
...
func MovementSystem(ents []Entity) {
	for i := range ents {
		...
		if v, ok := e.Components["velocity"].(*Velocity); ok {
			if p, ok := e.Components["position"].(*Position); ok {
				p.X += v.DX
				p.Y += v.DY
			}
		}
...
func (g *ExampleGame) Load(e *engine.Engine) error { return modules.RegisterAll(e) }

func (g *ExampleGame) Update(e *engine.Engine, _ engine.Tick) error {
	const s = 1
...
func setVelocity(e *engine.Engine, dx, dy int) {
	for i := range e.Entities {
		ent := &e.Entities[i]
		if v, ok := ent.Components["velocity"].(*engine.Velocity); ok {
			v.DX, v.DY = dx, dy
		}
	}
}
...
echo "Project ready for Go 1.24 in $D"